...
class ChromaMemoryStore:
    ...
    def __init__(self, embed_backend, path: str = "results/memory_db"):
        if chromadb is None:
            raise ImportError("chromadb is required for ChromaMemoryStore. Run: uv add chromadb")
    ...
    def retrieve(self, query: str, k: int, include_dormant: bool = False) -> list[MemoryItem]:
        ...
        for i, doc_id in enumerate(results["ids"][0]):
            ...
            alternatives = json.loads(meta["alternatives"]) if "alternatives" in meta else []
